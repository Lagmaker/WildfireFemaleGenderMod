    ...
    public class Entry extends EntryListWidget.Entry<WildfireBreastPresetList.Entry> {
        ...
        private Entry(final BreastPresetListEntry nInfo) {
            ...
            btnOpenGUI = new WildfireButton(0, 0, getRowWidth() - 6, itemHeight, Text.empty(), button -> {
                ...
                parent.getPlayer().getBreasts().updateWidth(nInfo.data.get(BreastPresetConfiguration.BREASTS_WIDTH));
                parent.getPlayer().getBreasts().updateHeight(nInfo.data.get(BreastPresetConfiguration.BREASTS_HEIGHT));
                parent.getPlayer().getBreasts().updateProjection(nInfo.data.get(BreastPresetConfiguration.BREASTS_PROJECTION));
                parent.getPlayer().getBreasts().updateBalance(nInfo.data.get(BreastPresetConfiguration.BREASTS_BALANCE));
                parent.getPlayer().getBreasts().updateShape(nInfo.data.get(BreastPresetConfiguration.BREASTS_SHAPE));
                parent.getPlayer().getBreasts().updateNipples(nInfo.data.get(BreastPresetConfiguration.BREASTS_NIPPLES));
                parent.getPlayer().getBreasts().updateNippleSize(nInfo.data.get(BreastPresetConfiguration.BREASTS_NIPPLE_SIZE));
                parent.getPlayer().updateBreastPhysics(nInfo.data.get(BreastPresetConfiguration.BREAST_PHYSICS));
                parent.getPlayer().updateBounceMultiplier(nInfo.data.get(BreastPresetConfiguration.BOUNCE_MULTIPLIER));
                parent.getPlayer().updateFloppiness(nInfo.data.get(BreastPresetConfiguration.FLOPPY_MULTIPLIER));
                parent.getPlayer().updateWobble(nInfo.data.get(BreastPresetConfiguration.WOBBLE_ENABLED));
                parent.getPlayer().updateWobbleIntensity(nInfo.data.get(BreastPresetConfiguration.WOBBLE_INTENSITY));
                parent.getPlayer().updateWobbleSpeed(nInfo.data.get(BreastPresetConfiguration.WOBBLE_SPEED));
                PlayerConfig.saveGenderInfo(parent.getPlayer());
            });
        }
