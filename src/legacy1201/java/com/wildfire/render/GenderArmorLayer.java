...
public class GenderArmorLayer<T extends LivingEntity, M extends BipedEntityModel<T>> extends GenderLayer<T, M> {
	...
	protected ModelBox lBoobArmor, rBoobArmor;
	protected ModelBox lTrim, rTrim;
	private float armorMeshSignature = Float.NaN;
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int packedLightIn, @Nonnull T ent, float limbAngle, float limbDistance, float partialTicks, float animationProgress, float headYaw, float headPitch) {
		...
		try {
			...
			resizeArmorMeshes(entityConfig);
	...
	private void resizeArmorMeshes(EntityConfig config) {
		var shape = config.getBreasts();
		float configuredSize = config.getBustSize();
		float signature = configuredSize * 31F + shape.getWidth() * 37F + shape.getHeight() * 41F
				+ shape.getProjection() * 43F + shape.getBalance() * 47F + shape.getShape().ordinal() * 53F;
		if(Float.compare(signature, armorMeshSignature) == 0) return;
		float leftScale = Math.max(0.05F, 1F + shape.getBalance());
		float rightScale = Math.max(0.05F, 1F - shape.getBalance());
		lBoobArmor = new ProfiledBreastModelBox(64, 32, -4F, configuredSize, shape.getWidth(), shape.getHeight(),
				shape.getProjection(), leftScale * 1.05F, shape.getShape(), 20, 21);
		rBoobArmor = new ProfiledBreastModelBox(64, 32, 0F, configuredSize, shape.getWidth(), shape.getHeight(),
				shape.getProjection(), rightScale * 1.05F, shape.getShape(), 24, 21);
		lTrim = new ProfiledBreastModelBox(64, 32, -4F, configuredSize, shape.getWidth(), shape.getHeight(),
				shape.getProjection(), leftScale * 1.057F, shape.getShape(), 20, 21);
		rTrim = new ProfiledBreastModelBox(64, 32, 0F, configuredSize, shape.getWidth(), shape.getHeight(),
				shape.getProjection(), rightScale * 1.057F, shape.getShape(), 24, 21);
		armorMeshSignature = signature;
	}
	...
	protected void renderBreastArmor(T entity, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int packedLightIn, BreastSide side) {
		...
		try {
			...
			ModelBox armor = side == BreastSide.LEFT ? lBoobArmor : rBoobArmor;
	...
	protected void renderArmorTrim(ArmorMaterial material, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int packedLightIn,
		...
		ModelBox trimModelBox = side == BreastSide.LEFT ? lTrim : rTrim;
		...
		VertexConsumer vertexConsumer = sprite.getTextureSpecificVertexConsumer(
				vertexConsumerProvider.getBuffer(TexturedRenderLayers.getArmorTrims()));
