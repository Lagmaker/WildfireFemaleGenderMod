...
public class WildfireModelRenderer {
   ...
   public static class TexturedQuad {
      ...
      public TexturedQuad(float u1, float v1, float u2, float v2, float texWidth, float texHeight, boolean mirrorIn, Direction directionIn, PositionTextureVertex... positionsIn) {
		 this(u1, v1, u2, v2, texWidth, texHeight, mirrorIn, directionIn.getUnitVector(), positionsIn);
	  }

	  public TexturedQuad(float u1, float v1, float u2, float v2, float texWidth, float texHeight, PositionTextureVertex... positionsIn) {
		 this(u1, v1, u2, v2, texWidth, texHeight, false, calculateNormal(positionsIn), positionsIn);
	  }

	  private TexturedQuad(float u1, float v1, float u2, float v2, float texWidth, float texHeight, boolean mirrorIn, Vector3f suppliedNormal, PositionTextureVertex... positionsIn) {
		 ...
		 this.normal = new Vector3f(suppliedNormal);
         if (mirrorIn) {
            this.normal.mul(-1.0f, 1.0f, 1.0f);
         }
      }

	  private static Vector3f calculateNormal(PositionTextureVertex[] positions) {
		 if(positions.length < 3) return new Vector3f(0, 0, -1);
		 PositionTextureVertex first = positions[0];
		 PositionTextureVertex second = positions[1];
		 PositionTextureVertex third = positions[2];
		 ...
		 Vector3f result = edgeA.cross(edgeB);
		 return result.lengthSquared() < 1.0E-6f ? new Vector3f(0, 0, -1) : result.normalize();
	  }
   }
}
